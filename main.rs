...
impl Item {
    fn buy_checker(&mut self, points : &mut f32, bought_vars : &mut HashMap<String,bool>, terminal: &mut Terminal) -> bool{
    ...
    fn buy(&mut self, points : &mut f32, bought_vars : &mut HashMap<String,bool>, terminal: &mut Terminal,points_modifier : &mut f32,points_gain : &mut u32,power_gain : &mut u32,max_power : &mut u32) {
...
fn main() {
    ...
    let mut item_list : [&mut Item; 4]= [&mut item,&mut item2,&mut item3,&mut item4];


        terminal.commands.insert("exit".to_string(), Box::new(|_game,_terminal,_item| {
        exit(0);
    }));    
    terminal.commands.insert("help".to_string(), Box::new(|game,terminal,_item| {
    ...
    terminal.commands.insert("shop".to_string(), Box::new(|game,terminal,item| {
        println!("working..");
        for item in item {
            item.buy(&mut game.points, &mut game.bought_vars, terminal, &mut game.points_modifier, &mut game.points_gain, &mut game.power_gain, &mut game.max_power); //WIP not done yet
        }
    }));
